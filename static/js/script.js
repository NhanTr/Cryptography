...
async function generateRSAKeys() {
  const pubKeyBox = document.getElementById("pub-key");
  const privKeyBox = document.getElementById("priv-key");
  const output = document.getElementById("result-output");

  pubKeyBox.value = "Đang tạo khóa...";
  privKeyBox.value = "Đang tạo khóa...";
  output.innerText = "Đang xử lý...";

  try {
    const response = await fetch("/generate-keys-rsa", { method: "POST" });
    const data = await response.json();

    if (data.status === "success") {
      pubKeyBox.value = data.public_key;
      privKeyBox.value = data.private_key;
      output.innerText = "Tạo cặp khóa RSA thành công!";
    } else {
      output.innerText = "Lỗi: " + data.message;
    }
  } catch (error) {
    console.error("Error generating RSA keys:", error);
    output.innerText = "Lỗi kết nối server.";
  }
}
...
async function encryptRSA() {
  ...
  const message = document.getElementById("asym-input").value;

  if (!pubKey || !message) {
    output.innerText =
      "Vui lòng tạo khóa Public Key và nhập dữ liệu cần mã hóa!";
    return;
  }

  output.innerText = "Đang mã hóa...";

  try {
    const response = await fetch("/encrypt-rsa", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ public_key: pubKey, message: message }),
    });
    const data = await response.json();

    if (data.status === "success") {
      output.innerText = data.result;
      // Tự động đẩy kết quả lên ô input để tiện test giải mã
      document.getElementById("asym-input").value = data.result;
    } else {
      output.innerText = "Lỗi: " + data.message;
    }
  } catch (error) {
...
async function decryptRSA() {
  const output = document.getElementById("result-output");
  const privKey = document.getElementById("priv-key").value;
  const ciphertext = document.getElementById("asym-input").value;

  if (!privKey || !ciphertext) {
    output.innerText =
      "Vui lòng tạo khóa Private Key và nhập dữ liệu (ciphertext) cần giải mã!";
    return;
  }
  ...
  try {
    const response = await fetch("/decrypt-rsa", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ private_key: privKey, ciphertext: ciphertext }),
    });
    const data = await response.json();
